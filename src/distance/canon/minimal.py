...
#! Only for L2 norm
def minimal_canon(reward: Reward, env: Env, max_iters=1e6) -> Reward:
  # convert to torch
  ...
  for i in range(int(max_iters)):
    ...
    # convergence checks
    if i%1000 == 0: # loss change over 1k iters is under 1e-6
      loss_val = loss.item()
      # print(f'{loss_val=:<24} delta={loss_frozen - loss_val}')
      loss_is_close = abs(loss_val - loss_frozen) < 1e-6
