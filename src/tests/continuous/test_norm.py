...
def test_norm():
  ...
  l2_norm = norm_cont(canonicalized, env_info.state_space, env_info.act_space)
  for _ in range(3):
    val2 = norm_cont(canonicalized, env_info.state_space, env_info.act_space)
    assert abs(l2_norm - val2) < 1e-2 * l2_norm

  l_inf_norm = norm_cont(canonicalized, env_info.state_space,
                         env_info.action_space, float('inf'))
  for _ in range(3):
    val2 = norm_cont(canonicalized, env_info.state_space,
                     env_info.action_space, float('inf'))
    assert abs(l_inf_norm - val2) < 1e-2 * l_inf_norm

  l1_weighted_norm = norm_cont(canonicalized, env_info.state_space,
                               env_info.action_space, 'weighted_1')
  for _ in range(3):
    val2 = norm_cont(canonicalized, env_info.state_space,
                     env_info.action_space, 'weighted_1')
